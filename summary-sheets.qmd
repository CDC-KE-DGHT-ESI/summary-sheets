...
cascade_uncond <- rio::import("data/Results/95_95_95/final/95-95-95 Cascade June 2025.xlsx",
                                   sheet = "Unconditional") |>
...
cascade_cond <- rio::import("data/Results/95_95_95/final/95-95-95 Cascade June 2025.xlsx",
                                   sheet = "Conditional") |>
rename(typo = Typology,
       county = County) %>%
# separate(prev_typo, c("county","typo")) |>
...
cascade <- bind_rows(cascade_uncond, cascade_cond) %>%
  ...
  mutate(indicator = case_when(
    indicator %in% c("Awareness","Aware_of_HIV_Status" ) ~ "Aware",
    indicator %in% c("On_ART", "On ART") ~ "On ART",
    indicator %in% c("VLS<200") ~ "Suppressed")
  ) %>%
  filter(!is.na(indicator)) %>%
  filter(age_band == "Overall")
...
num_typos <- filtered_data %>% distinct(typo) %>% pull(typo) %>% length()
plot_height <- if (num_typos > 3) 6 else 4
plot_width <- if (num_typos > 1) 8 else 6
facet_rows <- if (num_typos %in% c(1,2,3)) 1 else 2
...
cascade_plot <- long_data |>
  ...
  theme(
    ...
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    legend.position = "bottom",  # Conditional legend position
...
syphlis <- rio::import("data/Results/STI/STI Analysis Results_08_06_2025.xlsx",
                         sheet = "Syphilis_Active") |>
...
##
hebB <- rio::import("data/Results/STI/STI Analysis Results_08_06_2025.xlsx",
                         sheet = "HepB_Active") |>
  mutate(typo = case_when(is.na(Typology) ~ "All",
                          TRUE ~ Typology),
         county = case_when(is.na(County) ~ "All",
...
rename(Indicator = `Hepatitis B`) %>%
mutate(Indicator = case_when(is.na(Indicator) ~ NA_character_,
                             TRUE ~ Indicator)) %>%
mutate(sti = "Hepatitis B") %>%
filter(!is.na(Indicator))

##
hebC <- rio::import("data/Results/STI/STI Analysis Results_08_06_2025.xlsx",
                         sheet = "Hepc_active") |>
  mutate(typo = case_when(is.na(Typology) ~ "All",
                          TRUE ~ Typology),
         county = case_when(is.na(County) ~ "All",
...
##
chlamydia <- rio::import("data/Results/STI/STI Analysis Results_08_06_2025.xlsx",
                         sheet = "Chlamydia") |>
  mutate(typo = case_when(is.na(Typology) ~ "All",
                          TRUE ~ Typology),
         county = case_when(is.na(County) ~ "All",
...
##
gonorrhea <- rio::import("data/Results/STI/STI Analysis Results_08_06_2025.xlsx",
                         sheet = "Gonorhea") |>
  mutate(typo = case_when(is.na(Typology) ~ "All",
                          TRUE ~ Typology),
         county = case_when(is.na(County) ~ "All",
...
::: {layout-ncol="1"}
![Discrimination](`r hc_discrim_by_kp_age_file_path `)
:::
...
::: {layout-ncol="1"}
![Medication-Assisted Treatment](`r oat_by_kp_age_file_path_two `)
:::

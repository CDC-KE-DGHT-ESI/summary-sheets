...
prev_data <- rio::import("data/Results/PrevalenceandVLsupression02_12_2024.xlsx",
...
supp_data <- rio::import("data/Results/PrevalenceandVLsupression02_12_2024.xlsx",
...
prev_age <- rio::import("data/Results/PrevalenceandVLsupression02_12_2024.xlsx",
...
if (nrow(filtered_data) > 0) {
  ...
  labs(y = "Proportion (%)",
       x = "Age (years)",
       caption = "Error bars represent the 95% confidence interval (CI) \n
                  * Number suppressed due to small values"
       ) +
  theme_minimal() +
  scale_fill_manual(values = custom_colors) +
  scale_color_manual(values = custom_colors) +   
...
supp_age <- rio::import("data/Results/PrevalenceandVLsupression02_12_2024.xlsx",
...
filtered_data <- supp_age %>%
  ...
  tidyr::complete(COUNTY, typo, supp_age,  
      fill = list(COUNTY = county, supp_point = NA_real_, supp_lower = NA_real_, supp_upper = NA_real_))
...
plot_supp_200 <- ggplot(filtered_data, aes(x = factor(supp_age), 
  ...
  labs(y = "Proportion (%)",
       ...
       caption = "Error bars represent the 95% confidence interval (CI) \n
                   * Number suppressed due to small values \n
       Viral suppression is among all HIV positive individuals regardless of knowledge of HIV status or use of ART"
       ) +
  theme_minimal() +
  scale_fill_manual(values = custom_colors) +
  scale_color_manual(values = custom_colors) +   
...
cascade_uncond <- rio::import("data/Results/95_95_95/final/results_v1.xlsx",
...
cascade_cond <- rio::import("data/Results/95_95_95/final/results_v1.xlsx",
